...
// SR_CALC_ONLY drops every real host-display video backend (the dummy
// display never needs one). The Linux-no-deps path already falls through
// to "return nullptr" and is proven to work with the dummy display, so
// calc-only behaves identically on all platforms.
#ifndef SR_CALC_ONLY
#if defined(_WIN32)
...
{
// RPCS3 LOCAL PATCH: calc-only — no real video backend; behaves like the
// proven Linux-no-deps path (falls through to "return this").
#ifndef SR_CALC_ONLY
#if defined(_WIN32)
...
#endif
#endif // SR_CALC_ONLY (RPCS3 LOCAL PATCH)
