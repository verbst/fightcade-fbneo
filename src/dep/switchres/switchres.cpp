...
#include <fstream>
#include <string.h>
// RPCS3 LOCAL PATCH: std::getline(istream&, std::string&) is declared in
// <string>. libstdc++ pulls it in transitively via <fstream>; MSVC's STL
// does not, so include it explicitly for the std::getline call below.
#include <string>
#include <algorithm>
#include "switchres.h"
#include "log.h"
...
bool switchres_manager::parse_config(const char *file_name)
{
	...
	string line;
	// RPCS3 LOCAL PATCH: qualified std::getline (see <string> include above).
