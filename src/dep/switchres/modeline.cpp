...
// ---- RPCS3 LOCAL PATCH: MSVC portability ---------------------------------
// Upstream uses GCC statement-expressions ({ ... }) + __typeof__, which MSVC
// does not support. All call sites in this file pass plain side-effect-free
// modeline arithmetic, so the simple-ternary form is safe (no double-eval
// hazard). Keep the original GCC form on GCC/Clang.
#if defined(__GNUC__)
