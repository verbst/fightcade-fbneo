...
#include <string.h>
#include "display.h"
// ---- RPCS3 LOCAL PATCH: calc-only build ----------------------------------
// RPCS3 vendors switchres purely as a modeline calculator (the "dummy"
// display backend); it never switches a real host display. SR_CALC_ONLY
// drops the platform display backends (display_linux/windows/sdl2 + their
// custom_video_* deps on X11/DRM/ADL/ATI), so only the dummy path remains.
#ifndef SR_CALC_ONLY
#if defined(_WIN32)
...
#ifdef SR_CALC_ONLY
	// RPCS3 LOCAL PATCH: calc-only — no real host-display backend exists.
	// Any non-"dummy" request still resolves to the dummy backend so
	// modeline calculation always works without platform display code.
	display = new dummy_display(ds);
	return display;
#else
